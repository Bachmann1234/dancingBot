...
```
Microphone → BeatDetector (librosa) → Animator → Terminal
```

- `bot.py`: ASCII art frames (neutral, bob up, bob down)
- `animator.py`: Terminal rendering at 30 FPS, animation state
- `beat_detector.py`: Audio capture via sounddevice, onset detection via librosa
...
- Beat detection uses librosa's onset strength with threshold 0.3
